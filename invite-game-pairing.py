...
if __name__ == "__main__":
    ...
    # If the two LogID's are equivalent, they are the same game
            ...
            if team_A[1] == team_B[1] and team_A[1] not in invite_game_pairings['LogID'].tolist():
                if team_A[2] == "Blue":
                    invite_game_pairings = pd.concat([invite_game_pairings, pd.DataFrame({"Blue" : team_A[0], "Red": team_B[0],"LogID" : team_A[1], "GameMode" : team_A[3], "Map" : team_A[4]}, index = [0])], ignore_index=True)
                else:
                    invite_game_pairings = pd.concat([invite_game_pairings, pd.DataFrame({"Blue" : team_B[0], "Red": team_A[0],"LogID" : team_A[1], "GameMode" : team_A[3], "Map" : team_A[4]}, index = [0])], ignore_index=True)
        ...
        for index, row in invite_games.iterrows():
            if row["LogID"] not in invite_game_pairings["LogID"].values.tolist():
                ...
                if row["TeamColor"] == "Blue":
                    invite_game_pairings = pd.concat([invite_game_pairings, pd.DataFrame({"Blue" : row["TeamName"], "Red": "Unknown","LogID" : row["LogID"], "GameMode" : row["GameMode"], "Map" : row["Map"]}, index = [0])], ignore_index=True)
                if row["TeamColor"] == "Red":
                    invite_game_pairings = pd.concat([invite_game_pairings, pd.DataFrame({"Blue" : "Unknown", "Red": row["TeamName"],"LogID" : row["LogID"], "GameMode" : row["GameMode"], "Map" : row["Map"]}, index = [0])], ignore_index=True)
